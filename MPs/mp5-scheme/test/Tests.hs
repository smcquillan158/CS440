...
lambdaTests = 
  testCase "=P= Lambda tests (1 points)" $ do
    assertEval ["(lambda (x) (+ x 10))"] ["#<function:(\955 (x) ...)>"]
    assertEval ["((lambda (x) (+ (((lambda () (lambda () x)))) 10)) 20)"] ["30"]
    assertEval ["(lambda (X) ((lambda (f) (X (lambda (arg) ((f f) arg)))) (lambda (f) (X (lambda (arg) ((f f) arg))))))"] ["#<function:(\955 (X) ...)>"]
...
evalTests = 
  testCase "=P= Eval tests (1 points)" $ do
    ...
    assertEval ["(eval '(lambda (f x) (f x)))"] ["#<function:(\955 (f x) ...)>"]
